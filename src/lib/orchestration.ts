import { part, add, melody, pads, roots, shells, MELODY, phrases, closeVoicing, octave, annotate, type Part, type Treatment } from './orchestraMusic.ts';
import { createArtistTreatments } from './orchestraStudies.ts';
export * from './orchestraMusic.ts';
...
const legacyTreatments: Treatment[] = [
  {
...
export const TREATMENTS: Treatment[] = [...legacyTreatments, ...createArtistTreatments()];
const normalized = (name: string) => name.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/[^a-z0-9]/g, '');
const byName = new Map(TREATMENTS.map((t) => [normalized(t.name), t]));
export function treatmentForArtist(name: string): Treatment {
  const treatment = byName.get(normalized(name));
  if (!treatment) throw new Error('Missing orchestration study for ' + name);
  return treatment;
}
export const defaultTreatment = (name: string) => treatmentForArtist(name).id;
